import { pathApi } from "@/components/dashboard/admin/config";
...
export class Controller {
   get collection() {
      return `${pathApi}/panitia`;
   }
   get time() {
      return new TimeApp();
   }
   get storage() {
      return userStorage().panitia;
   }
}
export class Cruds extends Controller {
   ...
   async add(body: any): Promise<void> {
      body['id_priode'] = this.storage.id_priode ;
...
export class MainData extends Controller {
   ...
   get priode() {
      const data = userStorage().priode.data;

      const index = data.findIndex(
         (el) => el.id == this.data.id_priode
      );
   ...
   async init(): Promise<void> {
      ...
      const { data, status } = await get(
         `${this.collection}/${this.storage.id_priode}`
      );
      this.data.data = data;
